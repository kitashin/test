...
<button type="button" class="btn btn-danger"><a href="./">aaaaaa</a></button>
      <div>Mailing list</div>
    <section>

<?php
$cmd = '/usr/lib/mailman/bin/list_lists -V ' . $_GET[domain];
exec($cmd, $resultAry, $status);
echo "<div>";
echo '<ul class="nav nav-pills nav-stacked">';
for($i = 1; $i < count($resultAry); $i++){
$mlname = ereg_replace(' - .*',"", trim($resultAry[$i]));
echo '<li><a href="members.php?list_members=' . $mlname . '">' . $mlname . "</a></li>\n";
}
echo "</ul>";
echo "</div>";
